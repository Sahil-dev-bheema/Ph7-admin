...
export default function UserForget({
  ...
  const onSendOtp = async (e) => {
    ...
    try {
      setLoading(true);
      const { data } = await axiosInstance.post("/admin/forget-password", {
        email: email.trim(),
      });
console.log(data)
      if (data?.success) {
        ...
        setErr(data?.message || "Unable to send OTP.");
      }
    } catch (error) {
  ...
  const onVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (data?.success) {
        setMsg(data?.message || "OTP verified. Now set a new password.");
        setStep(3);
      } else {
  ...
  const onResetPassword = async (e) => {
    ...
    try {
      ...
      if (data?.success) {
        setMsg(data?.message || "Password reset successful. You can login now.");
        setTimeout(() => {
          onSuccess?.();
          onClose?.();
        }, 800);
  ...
  const onResend = async () => {
    ...
    try {
      ...
      if (data?.success) {
        setMsg(data?.message || "OTP resent. Please check your email.");
        setCooldown(30);
      } else {
        setErr(data?.message || "Unable to resend OTP.");
      }
    } catch (error) {
      setErr(error?.response?.data?.message || "Something went wrong.");
    } finally {
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/10 shadow-2xl">
          ...
          <div className="relative px-6 pt-6 pb-4">
            ...
            <div className="mt-5 flex items-center gap-2">
              ...
              <div
                className={`h-[2px] flex-1 ${step >= 2 ? "bg-white/70" : "bg-white/20"}`}
              />
              <StepPill active={step >= 2} label="OTP" />
              <div
                className={`h-[2px] flex-1 ${step >= 3 ? "bg-white/70" : "bg-white/20"}`}
              />
